...
export default function Page() {
  ...
  // Plain hard-cut slideshow (no crossfade / procedural overlay)
  const PlainSlideshow: React.FC<{ images: string[]; matchHeight: number }> = ({ images, matchHeight }) => {
    ...
    const heightStyle = isDesktop ? matchHeight : undefined;
    return (
      <div className="relative w-full border border-black overflow-hidden bg-white" style={{ height: isDesktop ? heightStyle : '60vh' }}>
        {images.map((src, i) => (
          <img
            key={src + i}
            src={src}
            alt="Venue still"
            className={`absolute inset-0 w-full h-full object-cover ${i === idx ? 'opacity-100' : 'opacity-0'} transition-none`}
            draggable={false}
  ...
  <canvas ref={traceCanvasRef} className="fixed inset-0 pointer-events-none z-30 mix-blend-multiply" aria-hidden="true" />
      ...
      <section id="about" className="py-16 px-4">
        <div className="w-full">
          ...
          <PlainSlideshow images={aboutImages} matchHeight={aboutTextHeight} />
          </div>
        </div>
      </section>
